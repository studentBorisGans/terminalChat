...
void *handle_client(void *arg)
{
    ...
    // Read the first client message to determine the type of request
    ...
    if (strncmp(buffer, "GET", 3) == 0)
    {
        ...
        // Check if the request is for the /rooms endpoint
        if (strstr(buffer, "GET /rooms") != NULL)
        {
            // Send chat rooms as JSON
            const char *json_response = "[{\"name\":\"General\",\"isPrivate\":false},{\"name\":\"Devs Only\",\"isPrivate\":true}]";
            send_http_response(client_socket, json_response, "application/json");
        }
        else
        {
    ...
    // If not HTTP, treat it as a chat protocol connection
    printf("Chat Protocol Request: %s\n", buffer);

    // Assume the first message is the username
    strncpy(username, buffer, sizeof(username) - 1);
    username[sizeof(username) - 1] = '\0'; // Null-terminate username
...
void send_http_response(int client_socket, const char *content, const char *content_type)
{
...
void add_cors_headers(int client_socket)
{
    const char *headers =
        "Access-Control-Allow-Origin: *\r\n"
